...
int cur_level = 0, id_num = 0, table_num = 0, cur_num, fun_num = 0, end_num = 0;
char cur_type[100], array[100], func_type[100], for_id[100];
bool in_array = false, forLoop = false;
string attri;
%}
/*types*/
%union {
...
program			: ID 
	  		 {
				...
				/*bool redeclare;
				printf("enter");
			 	redeclare = itemRedeclaration(0, tmp, table);
				printf("finish");
				if(redeclare == true)
				{
...
func_decl		: ID {
				...
				char * tmp = $1;
				/*bool redeclare;
			 	redeclare = itemRedeclaration(cur_level, tmp, table);
				if(redeclare == true)
				{
					printf("<Error> found in Line %d: symbol %s is redeclared\n", linenum, tmp);
				}
				...
				{*/	
					...
					table[table.size()-2].item.push_back(temp1);
				//}
			  }
			  compound_stmt
			  END
...
id_list			: id_list MK_COMMA ID{
				...
				/*bool redeclare;
			 	redeclare = itemRedeclaration(cur_level, $3, table);
				if(redeclare == true)
				{
					printf("<Error> found in Line %d: symbol %s is redeclared\n", linenum, $3);
				}
			...
			| ID{
	  			if(id_num == 0)
					cur_num = table[table_num-1].item.size();
				/*bool redeclare;
			 	redeclare = itemRedeclaration(cur_level, $1, table);
				if(redeclare == true)
				{
					printf("<Error> found in Line %d: symbol %s is redeclared\n", linenum, $1);
				}
...
compound_stmt		: BEG
				...
				if(fun_num > 0)	//already have table
				{
					...
					{
						items for_item;
						strcpy(for_item.name, for_id);
						strcpy(for_item.type, "for");
					}
				}
				else
				{	
...
for_stmt		: FOR ID {
	  			forLoop = true;
				/*bool redeclare;
			 	redeclare = itemRedeclaration(cur_level, $2, table);
				if(redeclare == true)
				{
					printf("<Error> found in Line %d: symbol %s is redeclared\n", linenum, $2);
				}
